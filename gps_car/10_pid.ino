...
int esc_pid(int speed) {
  ...
  const float MPH_TO_RPM_ENCODER = 174.7;  // 5280 ft\mile * 12 in\ft / (pi*5)in\rev_wheel / 60 minutes\hr * 2.6 rev_enc/rev_wheel
...
void check_encoder() {
  static bool timer_start = 0;
  if (rpm_encoder == 0 && armed) {
    static unsigned long no_encoder_countdown;
    if (!timer_start) {
